...
_APPDATA_DIR = os.path.join(
    os.environ.get("APPDATA", os.path.expanduser("~")), "vision-tool"
)
...
def _capability_profile_due(memory, now=None, force=False):
    ...
    if profile.get("status") == "running":
        ...
        if (
            started
            and int(now or time.time()) - started < CAPABILITY_REFRESH_STALE_AFTER
        ):
            return False
...
def _build_capability_probe_strategies():
    ...
    if model:
        _insert_model_strategies(
            strategies, model, "img", TINY_PNG_B64, "image/png", prompt=prompt
        )
...
def refresh_capability_profile(force=False, reason="manual", _already_marked=False):
    if os.environ.get("VISION_TOOL_DISABLE_CAPABILITY_REFRESH"):
        return False
    if not _already_marked and not _mark_capability_refresh_started(
    ...
    for item in results:
        ...
        entry = providers.setdefault(
            provider, {"ok": 0, "failed": 0, "limited": 0, "paid_ok": 0, "free_ok": 0}
        )
        ...
        def worker():
            try:
                refresh_capability_profile(
                    force=True, reason=reason, _already_marked=True
                )
            except Exception as exc:
                _finish_capability_refresh(
...
def build_multimodal_content(frames, prompt):
    ...
    for data, mime in frames:
        parts.append(
            {
                "type": "image_url",
                "image_url": {"url": f"data:{mime};base64,{b64(data)}"},
            }
        )
    return parts
...
OLLAMA_ENDPOINT = _local_http_base(
    os.environ.get("OLLAMA_HOST"), "http://127.0.0.1:11434"
)
LMSTUDIO_ENDPOINT = _local_http_base(
    os.environ.get("LMSTUDIO_BASE_URL"), "http://127.0.0.1:1234/v1"
)
OPENROUTER_ENDPOINT = _api_http_base(
    os.environ.get("OPENROUTER_BASE_URL"), "https://openrouter.ai/api/v1"
)
OPENAI_ENDPOINT = _api_http_base(
    os.environ.get("OPENAI_BASE_URL"), "https://api.openai.com/v1"
)
TOGETHER_ENDPOINT = _api_http_base(
    os.environ.get("TOGETHER_BASE_URL"), "https://api.together.xyz/v1"
)
DEEPINFRA_ENDPOINT = _api_http_base(
    os.environ.get("DEEPINFRA_BASE_URL"), "https://api.deepinfra.com/v1/openai"
)
XAI_ENDPOINT = _api_http_base(os.environ.get("XAI_BASE_URL"), "https://api.x.ai/v1")
ANTHROPIC_ENDPOINT = _api_http_base(
    os.environ.get("ANTHROPIC_BASE_URL"), "https://api.anthropic.com/v1"
)
COHERE_ENDPOINT = _api_http_base(
    os.environ.get("COHERE_BASE_URL"), "https://api.cohere.com/v2"
)
...
def call_lmstudio_multi(frames, prompt, model):
    payload = {
        "model": model,
        "messages": [
            {"role": "user", "content": build_multimodal_content(frames, prompt)}
        ],
        "max_tokens": 768,
    }
    req = urllib.request.Request(
...
def _call_openai_compatible(
    endpoint, key_name, b64data, mime, prompt, model, timeout=60
):
    payload = {
        "model": model,
        "messages": [
    ...
    req = urllib.request.Request(
        ...
        data=json.dumps(payload).encode(),
        headers={
            "Authorization": f"Bearer {CFG[key_name]}",
            "Content-Type": "application/json",
        },
    )
...
def _call_openai_compatible_multi(
    endpoint, key_name, frames, prompt, model, timeout=90
):
    payload = {
        "model": model,
        "messages": [
        ...
        content.append(
            {
                ...
                "source": {
                    "type": "base64",
                    "media_type": mime,
                    "data": b64(data) if isinstance(data, bytes) else data,
                },
            }
        )
    payload = {
...
def call_fireworks_multi(
    frames, prompt, model="accounts/fireworks/models/llama-v3p2-90b-vision-instruct"
):
    payload = {
        "model": model,
        "messages": [
...
def _build_strategies(kind, *args, prompt=""):
    if kind == "vid":
        ...
        s = [
            ...
            (
                "\u2606 Gemini 2.5 Flash",
                lambda: call_gemini_multi(frames, prompt, "gemini-2.5-flash"),
            ),
            (
                "\u2606 Gemini 3 Flash Preview",
                lambda: call_gemini_multi(frames, prompt, "gemini-3-flash-preview"),
            ),
            (
                "\u2606 Gemini 2.0 Flash",
                lambda: call_gemini_multi(frames, prompt, "gemini-2.0-flash"),
            ),
            (
                "\u2606 Gemini 2.0 Flash Lite",
                lambda: call_gemini_multi(frames, prompt, "gemini-2.0-flash-lite"),
            ),
            (
                "\u2606 Gemini 2.5 Pro",
                lambda: call_gemini_multi(frames, prompt, "gemini-2.5-pro"),
            ),
            (
                "\u2606 Gemini 3 Pro Preview",
                lambda: call_gemini_multi(frames, prompt, "gemini-3-pro-preview"),
            ),
            # OpenAI / Anthropic
            (
                "\u2606 OpenAI gpt-4o-mini",
                lambda: call_openai_multi(frames, prompt, "gpt-4o-mini"),
            ),
            (
                "\u2606 Anthropic Claude Sonnet",
                lambda: call_anthropic_multi(frames, prompt, "claude-sonnet-4-5"),
            ),
            # OpenAI-compatible vision providers
            (
                "\u2606 Together Kimi-K2.5",
                lambda: call_together_multi(frames, prompt, "moonshotai/Kimi-K2.5"),
            ),
            (
                "\u2606 DeepInfra Qwen2.5-VL-72B",
                lambda: call_deepinfra_multi(
                    frames, prompt, "Qwen/Qwen2.5-VL-72B-Instruct"
                ),
            ),
            (
                "\u2606 Cohere Command A Vision",
                lambda: call_cohere_multi(frames, prompt, "command-a-vision-07-2025"),
            ),
            ("\u2606 xAI Grok", lambda: call_xai_multi(frames, prompt, "grok-4.3")),
            # Azure AI Foundry
            (
                "\u2606 Azure DeepSeek-V4-Pro",
                lambda: call_azureai_multi(frames, prompt, "DeepSeek-V4-Pro"),
            ),
            (
                "\u2606 Azure gpt-4.1",
                lambda: call_azureai_multi(frames, prompt, "gpt-4.1"),
            ),
            (
                "\u2606 Azure gpt-4.1-mini",
                lambda: call_azureai_multi(frames, prompt, "gpt-4.1-mini"),
            ),
            (
                "\u2606 Azure gpt-4.1-nano",
                lambda: call_azureai_multi(frames, prompt, "gpt-4.1-nano"),
            ),
            (
                "\u2606 Azure gpt-4o",
                lambda: call_azureai_multi(frames, prompt, "gpt-4o"),
            ),
            (
                "\u2606 Azure gpt-4o-mini",
                lambda: call_azureai_multi(frames, prompt, "gpt-4o-mini"),
            ),
            (
                "\u2606 Azure gpt-5.1",
                lambda: call_azureai_multi(frames, prompt, "gpt-5.1"),
            ),
            (
                "\u2606 Azure gpt-5.4",
                lambda: call_azureai_multi(frames, prompt, "gpt-5.4"),
            ),
            (
                "\u2606 Azure gpt-5.4-mini",
                lambda: call_azureai_multi(frames, prompt, "gpt-5.4-mini"),
            ),
            (
                "\u2606 Azure gpt-5.4-nano",
                lambda: call_azureai_multi(frames, prompt, "gpt-5.4-nano"),
            ),
            (
                "\u2606 Azure Kimi-K2.6",
                lambda: call_azureai_multi(frames, prompt, "Kimi-K2.6"),
            ),
            (
                "\u2606 Azure Phi-4 multimodal",
                lambda: call_azureai_multi(frames, prompt, "Phi-4-multimodal-instruct"),
            ),
            # Groq
            (
                "\u2606 Groq Llama 4 Scout 17B",
                lambda: call_groq_multi(
                    frames, prompt, "meta-llama/llama-4-scout-17b-16e-instruct"
                ),
            ),
            # HuggingFace
            (
                "\u2606 HF Qwen3-VL-8B",
                lambda: call_hf_multi(frames, prompt, "Qwen/Qwen3-VL-8B-Instruct"),
            ),
            # Mistral AI
            (
                "\u2606 Mistral pixtral-large",
                lambda: call_mistral_multi(frames, prompt, "pixtral-large-latest"),
            ),
            # Fireworks AI
            (
                "\u2606 Fireworks Llama 3.2 90B Vision",
                lambda: call_fireworks_multi(
                    frames,
                    prompt,
            ...
            (
                "\u2606 ZAI Glm-4.5-Flash",
                lambda: call_zai_multi(frames, prompt, "glm-4.5-flash"),
            ),
        ]
    else:
        img_b64, mime = args
        s = [
            ...
            (
                "\u2606 Gemini 2.5 Flash",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-2.5-flash"),
            ),
            (
                "\u2606 Gemini 3 Flash Preview",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-3-flash-preview"),
            ),
            (
                "\u2606 Gemini 2.0 Flash",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-2.0-flash"),
            ),
            (
                "\u2606 Gemini 2.0 Flash Lite",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-2.0-flash-lite"),
            ),
            (
                "\u2606 Gemini 2.5 Pro",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-2.5-pro"),
            ),
            (
                "\u2606 Gemini 3 Pro Preview",
                lambda: call_gemini(img_b64, mime, prompt, "gemini-3-pro-preview"),
            ),
            # OpenAI / Anthropic
            (
                "\u2606 OpenAI gpt-4o-mini",
                lambda: call_openai(img_b64, mime, prompt, "gpt-4o-mini"),
            ),
            (
                "\u2606 Anthropic Claude Sonnet",
                lambda: call_anthropic(img_b64, mime, prompt, "claude-sonnet-4-5"),
            ),
            # OpenAI-compatible vision providers
            (
                "\u2606 Together Kimi-K2.5",
                lambda: call_together(img_b64, mime, prompt, "moonshotai/Kimi-K2.5"),
            ),
            (
                "\u2606 DeepInfra Qwen2.5-VL-72B",
                lambda: call_deepinfra(
                    img_b64, mime, prompt, "Qwen/Qwen2.5-VL-72B-Instruct"
                ),
            ),
            (
                "\u2606 Cohere Command A Vision",
                lambda: call_cohere(img_b64, mime, prompt, "command-a-vision-07-2025"),
            ),
            ("\u2606 xAI Grok", lambda: call_xai(img_b64, mime, prompt, "grok-4.3")),
            # Azure AI Foundry
            (
                "\u2606 Azure DeepSeek-V4-Pro",
                lambda: call_azureai(img_b64, mime, prompt, "DeepSeek-V4-Pro"),
            ),
            (
                "\u2606 Azure gpt-4.1",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-4.1"),
            ),
            (
                "\u2606 Azure gpt-4.1-mini",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-4.1-mini"),
            ),
            (
                "\u2606 Azure gpt-4.1-nano",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-4.1-nano"),
            ),
            (
                "\u2606 Azure gpt-4o",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-4o"),
            ),
            (
                "\u2606 Azure gpt-4o-mini",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-4o-mini"),
            ),
            (
                "\u2606 Azure gpt-5.1",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-5.1"),
            ),
            (
                "\u2606 Azure gpt-5.4",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-5.4"),
            ),
            (
                "\u2606 Azure gpt-5.4-mini",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-5.4-mini"),
            ),
            (
                "\u2606 Azure gpt-5.4-nano",
                lambda: call_azureai(img_b64, mime, prompt, "gpt-5.4-nano"),
            ),
            (
                "\u2606 Azure Kimi-K2.6",
                lambda: call_azureai(img_b64, mime, prompt, "Kimi-K2.6"),
            ),
            (
                "\u2606 Azure Phi-4 multimodal",
                lambda: call_azureai(
                    img_b64, mime, prompt, "Phi-4-multimodal-instruct"
                ),
            ),
            # Groq
            (
                "\u2606 Groq Llama 4 Scout 17B",
                lambda: call_groq(
                    img_b64, mime, prompt, "meta-llama/llama-4-scout-17b-16e-instruct"
                ),
            ),
            # HuggingFace
            (
                "\u2606 HF Qwen3-VL-8B",
                lambda: call_hf_inference(
                    img_b64, mime, prompt, "Qwen/Qwen3-VL-8B-Instruct"
                ),
            ),
            # Mistral AI
            (
                "\u2606 Mistral pixtral-large",
                lambda: call_mistral(img_b64, mime, prompt, "pixtral-large-latest"),
            ),
            # Fireworks AI
            (
                "\u2606 Fireworks Llama 3.2 90B Vision",
                lambda: call_fireworks(
                    img_b64,
                    mime,
                    prompt,
                    "accounts/fireworks/models/llama-v3p2-90b-vision-instruct",
            ...
            (
                "\u2606 ZAI Glm-4.5-Flash",
                lambda: call_zai(img_b64, mime, prompt, "glm-4.5-flash"),
            ),
        ]
    return s
...
def analyze(file_path, prompt="", model=None):
    ...
    if model:
        _insert_model_strategies(
            ...
            model,
            "vid" if vid else "img",
            *(frames if vid else (img_b64, mime)),
            prompt=prompt,
        )
    ...
    if skipped:
        print(
            f"KEYS: Skipped {skipped}/{before} backends (missing API key)",
            file=sys.stderr,
            flush=True,
        )

    backend_memory = _load_backend_memory()
    strategies, memory_skipped = _filter_strategies_by_memory(
        strategies, backend_memory
    )
    if memory_skipped:
        print(
    ...
    if not strategies:
        ...
        raise RuntimeError(
            "No backends available — configure at least one API key (python setup.py)"
        )
    ...
    print(
        f"FALLBACK: {len(remaining)} backends in parallel", file=sys.stderr, flush=True
    )
    pool = concurrent.futures.ThreadPoolExecutor(max_workers=len(remaining))
    futs = {
        pool.submit(
            lambda f=fn, n=name: (n, _call_with_timeout(f, PER_CALL_TIMEOUT))
        ): name
...
def main():
    ...
    parser.add_argument(
        "--model", "-m", help="Custom model name (auto-routes to best provider)"
    )
    parser.add_argument(
        "--models",
        "-l",
        action="store_true",
        help="List available vision backends and exit",
    )
    parser.add_argument(
        "--refresh-profile",
        action="store_true",
        help="Refresh provider free/paid capability profile now",
    )
