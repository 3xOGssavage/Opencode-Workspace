...
def _promote_user_env(keys):
    r"""Read each named env var from HKCU\Environment (User scope) and
    promote it into the current process's environment when missing or empty.

    Windows does not auto-propagate HKCU\Environment values to existing or
    child processes after a change unless they are explicitly refreshed. This
    helper lets the MCP server pick up a freshly-updated User-scope
    GEMINI_API_KEY (etc.) without requiring opencode itself to be restarted.

    Behavior:
      - If the env var already has a non-empty value, leave it alone.
      - Otherwise, look it up in HKCU\Environment. If found, set it in os.environ.
      - Silently no-op on non-Windows platforms or if the registry call fails.
    ...
    try:
        with winreg.ConnectRegistry(None, winreg.HKEY_CURRENT_USER) as hkcu:
            with winreg.OpenKey(hkcu, r"Environment") as env_key:
                for k in keys:
                    existing = os.environ.get(k)
                    if existing:
                        continue
                    try:
                        val, _ = winreg.QueryValueEx(env_key, k)
...
# Promote known vision-provider env vars from User scope BEFORE vision_proxy
# is first imported. vision_proxy.load_config() reads os.environ at call time,
# but env vars set here are inherited by any subprocess python spawns as well.
_promote_user_env(
    [
        "GEMINI_API_KEY",
        "OPENAI_API_KEY",
